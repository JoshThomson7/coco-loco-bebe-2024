...
(function ($, root, undefined) {
	...
	$.fn.closeOnClickOutside = function (targetSelector, containerSelector) {
		$(document).on('click', function (evt) {
			var $target = $(targetSelector);
			var $container = $(containerSelector);
	
			if (!$target.is(evt.target) && $target.has(evt.target).length === 0) {
				$target.removeClass('active');
				$container.removeClass('popped');
			}
		});
	}
